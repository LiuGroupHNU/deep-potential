...
def obtain_frame(fname):
    ...
    return atom_names, atom_numbs, np.array(atom_types), np.array([cell]), np.array([coord]), \
           np.array([energy]), np.array([force]), virial

#return atom_names, atom_numbs, atom_types, np.array(all_cells), np.array(all_coords), \
#       np.array(all_energies), np.array(all_forces), np.array(all_virials)

# atom_names, atom_numbs, atom_types, cell, coord, energy, force,virial = \
#     obtain_frame('/home/smc/package/dpdata_siesta/tests/siesta/siesta_output')
