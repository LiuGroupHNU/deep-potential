...
class TestSIESTASinglePointEnergy:
    ...
    def test_cell(self):
        ...
        cells = self.system.data['cells']
        idx = 0
        for ii in range(len(cells)):
            for jj in range(len(cells[ii])):
                self.assertAlmostEqual(cells[ii][jj], float(ref_cell[idx]))
                idx += 1
    ...
    def test_coord(self):
        ...
        fp.close()
        coords = self.system.data['coords']
        idx = 0
        for ii in range(len(coords)):
            for jj in range(len(coords[ii])):
                self.assertAlmostEqual(coords[ii][jj], float(ref_coord[idx]))
                idx += 1
    ...
    def test_force(self):
        ...
        fp.close()
        forces = self.system.data['forces']
        idx = 0
        for ii in range(len(forces)):
            for jj in range(len(forces[ii])):
                self.assertAlmostEqual(forces[ii][jj], float(ref_force[idx]))
                idx += 1
    ...
    def test_viriale(self):
        ...
        fp.close()
        virials = self.system.data['virials']
        idx = 0
        for ii in range(len(virials)):
            for jj in range(len(virials[ii])):
                self.assertAlmostEqual(virials[ii][jj], float(ref_virial[idx]) * toViri * vol)
                idx += 1
